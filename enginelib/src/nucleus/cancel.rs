...
/// the dedup entry and set the rescan flag instead, dropping the task back to
/// "persisted only" so the loader rewinds and re-enqueues it. Either way the
/// task is never stranded.
pub fn cancel(api: Arc<ServerAPI>, task_type: Identifier, task_id: String) -> Result<(), Error> {
    ...
    match queue.0.try_send((*stored_task).clone()) {
        ...
        Ok(()) => Ok(()),
        // Channel full/closed: clear the dedup entry, then signal the loader to
        // rescan so the still-persisted record is re-enqueued. Never stranded.
        Err(_) => {
