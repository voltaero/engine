use engine::{
    ...
    get_auth, get_uid, proto,
    task_id::{mint_task_instance_id, parse_task_key},
};
...
#[tonic::async_trait]
impl proto::engine_server::Engine for BackendEngineService {
    async fn check_auth(
        &self,
    ...
    async fn delete_task(
        ...
        let result = match data.state() {
            proto::TaskState::Processing => delete_task_from_collection(
            ...
            proto::TaskState::Solved => delete_task_from_collection(
    ...
    async fn get_tasks(
        ...
        let q: Vec<proto::Task> = match data.state() {
            proto::TaskState::Processing => match api
                .executing_tasks
                .tasks
            ...
            proto::TaskState::Queued => match api
                .task_queue
                .tasks
                .get(&(data.namespace.clone(), data.task.clone()))
            {
            ...
            proto::TaskState::Solved => match api
                .solved_tasks
                .tasks
                .get(&(data.namespace.clone(), data.task.clone()))
            {
    ...
    async fn aquire_task(
        ...
        let mut exec_tsks = api.executing_tasks.tasks.get(&key).cloned().unwrap_or_default();
    ...
    async fn publish_task(
        ...
        let mem_tsk = api.executing_tasks.tasks.get(&key).cloned().unwrap_or_default();
        ...
        let mut mem_solv = api.solved_tasks.tasks.get(&key).cloned().unwrap_or_default();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let listener = TcpListener::bind(api.cfg.config_toml.host.as_str()).await?;
    ...
    let _registration_task =
        registration.map(|registration| spawn_registration(registration, registration_shutdown));
